...
{ 
	...
	if (!function_is_undefined()) 
		cout << "Целевая фукнция не ограничена, данная задача решений не имеет" << endl; 
	else { 
		cout << "f(x) = " << func << "" << endl; 
		for (i = 0; i < num_l; i++) { 
			cout << "x" << bv[i][0] + 1 << " = " << bv[i][1] << endl; 
		} 
		cout << "Все вычисления были записаны в файл table.txt" << endl; 
	} 
} 
...
{ 
	int i, j; 
	if (!it_num) { 
		table << "Задана целевая функция:" << endl; 
		...
		f_x << "=> " << minmax << "" << endl; 
		table << f_x.str(); 
		table << "И система ограничений:" << endl; 
		...
		table << "Решим данную задачу на " << min_or_max << " методом симплексных таблиц.Построим первый опорный план:" << endl; 
	} 
	for (i = 0; i < num_l; i++) { 
	...
	if (plane_is_valid()) { 
		if (plane_is_valid() && function_is_undefined()) 
			table << " Данный план является оптимальным и не требует улучшения. Решение найдено." << endl; 
		...
		table << "" << num_of_plane.str() << " не является оптимальным, поскольку в индексной строке присутствуют " << ln_or_gn << " элементы. Erо необходимо улучшить." << endl; 
	} 
}